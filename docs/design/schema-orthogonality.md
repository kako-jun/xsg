...
> 注: 本書が扱う `animation` / `blink`（および「Animation の実装」「blink + animation の実装」節の擬似コード）は **v1.1 予定で現行未実装**です。schema・型には残っていますが、現行レンダラー（`NodeRenderer`）は描画しません。以下は設計記録として保持しています。
